<x-app-layout title="Selamat Datang">
  ...
  {{-- ========================================== --}}
  {{-- NOTIFIKASI SYSTEM (VIA COMPONENT) --}}
  {{-- ========================================== --}}
  @auth
    <x-user-status-alert />
  @endauth
  ...
  <section class="relative min-h-[70vh] md:min-h-[80vh] flex items-center justify-center overflow-hidden">
    ...
    <div class="relative z-10 container mx-auto mt-38 md:mt-28 px-4 sm:px-6 lg:px-8 py-32 md:py-48 text-center">
      ...
      <p
        ...
        Wadah resmi pembinaan prestasi, karakter, dan kekeluargaan Keluarga Silat Nasional Indonesia Perisai
        Diri Cabang
          ...
          <div class="prose dark:prose-invert text-slate-600 dark:text-slate-300">
            <p class="mb-4">
              Keluarga Silat Nasional Indonesia Perisai Diri didirikan oleh <strong>R.M. Soebandiman
                Dirdjoatmodjo</strong> pada tanggal 2 Juli 1955 di Surabaya. Beliau adalah putra
              bangsawan Paku Alam
            ...
            <p class="mb-4">
              ...
              Indonesia ditambah
              dengan aliran Shaolin dari negeri Tiongkok. Teknik ini diramu menjadi teknik yang efektif,
              cepat, dan
              tepat dengan motto <em>"Pandai Silat Tanpa Cedera"</em>.
            </p>
            <p>
              Di Kabupaten Tasikmalaya, Perisai Diri terus berkembang mencetak atlet berprestasi dan
              membentuk karakter
  ...
  {{-- ========================================== --}}
  {{-- SECTION DOKUMENTASI & ARSIP KEGIATAN --}}
  {{-- ========================================== --}}
  {{-- LOGIC: Cek Login & Status Verifikasi --}}
          ...
          @foreach ($archives as $archive)
            <div
              ...
              <div class="relative h-56 overflow-hidden">
                <img src="{{ $archive->getFirstMediaUrl('thumbnail') }}" alt="{{ $archive->title }}"
                  class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110">
                <div class="absolute inset-0 bg-linear-to-t from-black/60 to-transparent opacity-60">
                </div>
                <div class="absolute bottom-3 left-4">
                  <span class="inline-block px-2 py-1 bg-blue-600/90 text-white text-xs font-semibold rounded-md">
                    {{ $archive->date->format('d F Y') }}
                  </span>
                </div>
              </div>
              ...
              <div class="p-6 flex-1 flex flex-col">
                ...
                <div class="flex flex-wrap gap-2 mt-auto pt-4 border-t border-slate-100 dark:border-slate-700">
                  @if ($archive->link_google_drive)
                    <a href="{{ $archive->link_google_drive }}" target="_blank"
                  ...
                  @if ($archive->link_instagram)
                    <a href="{{ $archive->link_instagram }}" target="_blank"
                  ...
                  @if ($archive->link_tiktok)
                    <a href="{{ $archive->link_tiktok }}" target="_blank"
  ...
  <section class="py-16 bg-slate-50 dark:bg-slate-950">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
      <div class="text-center mb-10">
        ...
        <p class="mt-2 text-slate-600 dark:text-slate-400">Kunjungi sekretariat cabang kami untuk informasi
          lebih
          lanjut.</p>
      </div>
    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <x-lightbox :images="$welcomeImages">
        <div class="columns-2 md:columns-4 gap-4 space-y-4">
          ...
          {{-- Loop langsung data dari Controller --}}
          @foreach ($welcomeImages as $index => $img)
            <div class="group relative break-inside-avoid overflow-hidden rounded-2xl shadow-lg cursor-pointer"
              @click="openLightbox({{ $index }})">

              {{-- Gambar Utama --}}
              {{-- Perhatikan: Kita pakai asset() di sini karena di controller isinya cuma string path --}}
              <img class="h-auto w-full object-cover transition-transform duration-500 group-hover:scale-110"
                src="{{ asset($img['src']) }}" alt="{{ $img['alt'] }}">

              {{-- Overlay Gelap & Ikon saat Hover --}}
              <div
                class="absolute inset-0 bg-black/40 opacity-0 transition-opacity duration-300 group-hover:opacity-100 flex items-center justify-center">
                <div class="transform translate-y-4 transition-transform duration-300 group-hover:translate-y-0">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-white drop-shadow-md" fill="none"
