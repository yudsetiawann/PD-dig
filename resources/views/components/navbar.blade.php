<header x-data="{ mobileOpen: false, scrolled: false }" @scroll.window="scrolled = (window.scrollY > 10)">
  ...
  <nav x-cloak x-show="mobileOpen" x-transition:enter="transition ease-out duration-300 transform"
    ...
    class="fixed top-0 right-0 w-full max-w-xs z-60 bg-white dark:bg-slate-900 shadow-xl md:hidden  rounded-md">
    ...
    <div class="p-4 space-y-4 overflow-y-auto"> {{-- Tambahkan overflow-y-auto jika konten bisa panjang --}}
      <nav class="space-y-2">
        <a href="{{ route('home') }}"
      ...
      @auth
        ...
        <div class="space-y-2 mt-4"> {{-- Added mt-4 for spacing --}}
          @if (Auth::user()->isAdmin())
            <a href="{{ route('filament.admin.pages.dashboard') }}"
      ...
      <div class="my-1.5 h-px bg-slate-200 dark:bg-slate-700 mt-4"></div> {{-- Added mt-4 for spacing --}}
